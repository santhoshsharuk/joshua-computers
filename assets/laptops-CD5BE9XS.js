import{q as h,c as g,d as f,o as p,g as v}from"./firebase-By_hAIbR.js";const l=document.getElementById("products-grid"),x=document.getElementById("product-count"),c=document.getElementById("clear-filters");let a=[];async function y(){try{const n=h(g(f,"products"),p("createdAt","desc")),e=await v(n);a=[],e.forEach(t=>{a.push({id:t.id,...t.data()})}),i(a);const o=new URLSearchParams(window.location.search).get("condition");o&&(document.querySelectorAll(".filter-condition").forEach(t=>{t.value===o&&(t.checked=!0)}),d())}catch(n){console.error("Error loading products:",n),l.innerHTML='<div class="col-span-full text-center text-red-500">Error loading products.</div>'}}function i(n){if(l.innerHTML="",x.textContent=`${n.length} Products`,n.length===0){l.innerHTML='<div class="col-span-full text-center text-gray-500 py-20">No products match your filters.</div>';return}n.forEach(e=>{const r=document.createElement("div");r.className="card-premium group flex flex-col";const o=`${window.location.origin}/product.html?id=${e.id}`,s=`https://wa.me/916382283294?text=${encodeURIComponent(`Hi Joshua Computers!
...
Contact: 6382283294`)}`;r.innerHTML=`
    ...
    `,l.appendChild(r)})}function d(){const n=Array.from(document.querySelectorAll(".filter-brand:checked")).map(t=>t.value),e=Array.from(document.querySelectorAll(".filter-ram:checked")).map(t=>t.value),r=Array.from(document.querySelectorAll(".filter-condition:checked")).map(t=>t.value),o=a.filter(t=>{const s=n.length===0||n.includes(t.brand),m=e.length===0||e.includes(t.ram),u=r.length===0||r.includes(t.condition);return s&&m&&u});i(o)}document.querySelectorAll('input[type="checkbox"]').forEach(n=>{n.addEventListener("change",d)});c&&c.addEventListener("click",()=>{document.querySelectorAll('input[type="checkbox"]').forEach(n=>n.checked=!1),i(a)});y();
